/*    
      ...
      nate@pobox.com, http://www.pobox.com/~nate
		...
		Diederick Huijbers, 2011
		info@roxlu.com
		- minor changes to make this work with openFrameworks + Blender exports.
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    for (i = 0; i < nummaterials; i++) {
		...
		model->materials[i].map_kd = NULL; // Diederick
    }
			...
			case 'm': {
				...
				if(strncmp(buf, "map_Kd", 6) == 0) {
					...
					while(true) {	
						...
						if (file_char == '\n' || file_char == '\r') {
							texture_file[char_count] = '\0';
							
							// right trim.
							int trim_pos = char_count;
							int k = 0;
							for(k = char_count-1; k > 0; --k) {
								char d = texture_file[k];
								if(texture_file[k] == ' ') {
									trim_pos--;
								}
								else {
							...
							if(trim_pos != char_count) {
								texture_file[trim_pos] = '\0';
							}
							
							// set the diffuse map file name.
							model->materials[nummaterials].map_kd = (char*)malloc(sizeof(char) * strlen(texture_file));
							strcpy(model->materials[nummaterials].map_kd,texture_file);
							break;
						}
						else if(count < 1024 && count > 0) {
							texture_file[char_count] = file_char;
							char_count++;
						}
						else if(count > 1024) {
							printf("Diffuse map filename is too large!\n");
							break;
						}
						count++;
					}
...
glmFirstPass(GLMmodel* model, FILE* file) 
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        switch(buf[0]) {
			...
			// o = object_name is the user-defined object name. There is no default.
			// http://www.martinreddy.net/gfx/3d/OBJ.spec
			case 'o':
			// group  
			case 'g': {              
				fgets(buf, sizeof(buf), file); 		// eat up rest of line 
				//#define SINGLE_STRING_GROUP_NAMES // !!!: added as test!
